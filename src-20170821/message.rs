...
impl Message for DiscoverMsg {
	...
	fn process(&mut self, ca: &mut CellAgent, port_no: PortNo) -> Result<()> {
		...
		let new_tree_id = self.get_tree_id(self.payload.get_tree_name())?.clone();
		let senders_index = self.payload.get_index();
		let children = &mut HashSet::new();
		let exists = ca.exists(&new_tree_id);
		let status = if exists { traph::PortStatus::Pruned } else { traph::PortStatus::Parent };
		// update_traph adds the tree back into ca.stacked_trees
		let tree = ca.update_traph(&new_tree_id, port_number, status,
		...
		let my_index = tree.get_table_index();
		...
		let mut user_mask = DEFAULT_USER_MASK.all_but_port(PortNumber::new(port_no, ca.get_no_ports()).chain_err(|| ErrorKind::MessageError)?);
		{
			let boundary_mask = ca.get_boundary_port_mask();
			user_mask = user_mask.and(boundary_mask.not()); // Don't send discover on boundary port
		}
...
impl Message for DiscoverDMsg {
	...
	fn process(&mut self, ca: &mut CellAgent, port_no: PortNo) -> Result<()> {
		...
		let tree =
		{
			let stacked_trees = ca.get_stacked_trees();
			let locked = stacked_trees.lock().unwrap();
			ca.clone().get_tree(locked, &tree_id, None).chain_err(|| ErrorKind::MessageError)? 
		// update_traph adds the tree back into ca.stacked_trees
		};
		ca.update_traph(&tree_id, port_number, traph::PortStatus::Child,
...
//		}
		Tree(cell_id: CellID, tree_id: TreeID) {
			display("Message: CellAgent {} doesn't know of tree {}", cell_id, tree_id)
		}
		TreeMapMissing(reason: bool) {
			display("Message: No tree map")
		}
		TreeMapEntry(tree_name: String) {
			display("Message: No tree named {} in map", tree_name)
		}		
	}
}
