...
use serde_json;
use eval::{Value, to_value};

use config::{MAX_ENTRIES, MsgID, PathLength, PortNo, TableIndex};
use container::Service;
use gvm_equation::{GvmEquation, GvmVariables};
use message::{DiscoverMsg, DiscoverDMsg, StackTreeMsg, SetupVMsMsg, Message, MsgDirection, MsgType};
...
impl CellAgent {
	...
	pub fn initialize(&mut self, cell_type: CellType, ca_from_pe: CaFromPe) -> Result<()> {
		...
		let port_number_0 = PortNumber::new(0, self.no_ports)?;
		let other_index = 0;
		let hops = 0;
		let path = None;
	...
	pub fn create_vms(&mut self, service_sets: Vec<Vec<Service>>) -> Result<()> {
		let up_traph_id = UpTraphID::new(&format!("Up:{}+{}", self.cell_id, self.up_traphs_clist.len()))?;
...
// Here's the end of the transaction
	...
	fn get_vars(&self, traph: &Traph, vars: &GvmVariables) -> Result<HashMap<String,Value>> {
		let mut var_map = HashMap::new();
		for var in vars.iter() {
			if var == "hops" {
				var_map.insert(var.clone(), to_value(traph.get_hops()?));
			} else {
			...
			match ca_from_pe.recv()? {
				...
				PeToCaPacket::Packet(port_no, packet) => {
					...
					if let Some(packets) = packet_assembler.add(packet) {
						let (msg_type, serialized_msg) = MsgType::get_type_serialized(packets)?;
	...
	fn get_msg(&self, msg_type: MsgType, serialized_msg: String) -> Result<Option<Box<Message>>> {
		Ok(match msg_type {
			MsgType::Discover => Some(Box::new(serde_json::from_str::<DiscoverMsg>(&serialized_msg)?)),
			MsgType::DiscoverD => Some(Box::new(serde_json::from_str::<DiscoverDMsg>(&serialized_msg)?)),
			MsgType::StackTree => Some(Box::new(serde_json::from_str::<StackTreeMsg>(&serialized_msg)?)),
	...
	fn port_connected(&mut self, port_no: PortNo, is_border: bool) -> Result<()> {
		...
		if is_border {
			...
			let msg = StackTreeMsg::new(&tree_id, &self.my_tree_id)?;
			let bytes = Serializer::serialize(&msg)?;
			let other_index = 0;
			let port_no_mask = Mask::all_but_zero();
			let my_index = self.my_entry.get_index();
			let packets = Packetizer::packetize(bytes, MsgDirection::Leafward)?;
...
//				self.ca_to_pe.send(CaToPePacket::Packet((my_index, port_no_mask, *packet)))?;			
			...
			let bytes = Serializer::serialize(&msg)?;
			let packets = Packetizer::packetize(bytes, direction)?;
	...
	pub fn forward_discover(&mut self, discover_msgs: &Vec<DiscoverMsg>, mask: Mask) -> Result<()> {
		...
		for msg in discover_msgs.iter() {
			...
			let bytes = Serializer::serialize(msg)?;
			let packets = Packetizer::packetize(bytes, direction)?;
			self.send_msg(&self.connected_tree_id, packets, mask)?;
