...
impl PacketEngine {
	...
	fn forward(&self, recv_port_no: PortNo, entry: RoutingTableEntry, user_mask: Mask, packet: Packet) 
		...
		if header.is_rootcast() {
			...
			if is_stack_msg { println!("PacketEngine {}: forwarding on ports {:?}, {} packet {}", self.cell_id, port_nos, entry, packet); }
			for port_no in port_nos.iter() {
				if let Some(other_index) = other_indices.get(*port_no as usize) {
					...
					return Err(ErrorKind::Index(self.cell_id.clone(), *port_no).into());
				}
			}
		}
		Ok(())
...
error_chain! {
	...
	errors { PacketEngineError
		Index(cell_id: CellID, port_no: PortNo) {
			display("No index for port {} on cell {}", port_no, cell_id)
		}
