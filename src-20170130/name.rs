...
impl Name {
	fn new(n: &str) -> Result<Name,NameError> {
		...
		let msg1 = format!("NameError: '{}' contains blanks.", n);
		let msg2 = format!("NameError: '{}' more than {} characters", n, MAX_NAME_SIZE);
		match n.find(' ') {
			Some(_) => { Err(NameError::invalid_character(n)) },
			None => {
				if n.len() <= MAX_NAME_SIZE { 
					for c in n.char_indices() { name[c.0] = c.1; } 
					Ok(Name { name: name, })
				} else { 
					Err(NameError::name_too_long(n)) }
			}
		}		
	}
	...
	fn add_component(&self, s: &str) -> Result<Self,NameError> {
		...
		match s.find(' ') {
			Some(_) => { Err(NameError::invalid_character(s)) },
			None => {
				if self.len() + s_plus.len() <= MAX_NAME_SIZE { 
					...
					Err(NameError::name_too_long(&format!("{}", n.len() + s.len())))
				}
			}
		}
	}
...
impl NameError { 
	fn invalid_character(s: &str) -> NameError {
		NameError { msg: format!("NameError: '{}' contains blanks.", s)	}
	} 
	fn name_too_long(s: &str) -> NameError {
		NameError { msg: format!("NameError: '{}' more than {} characters", s, MAX_NAME_SIZE) }
	}
}
