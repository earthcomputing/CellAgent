...
//  pub fn get_is_connected(&self) -> Arc<AtomicBool> { self.is_connected.clone() }
    ...
    pub fn link_channel(&self, simulated_port_or_ecnl_port: &Either<SimulatedPort, Arc<ECNL_Port>>,
        ...
        thread::Builder::new().name(thread_name).spawn( move || {
            update_trace_header(child_trace_header);
            let _ = port.listen_link_loop(&simulated_port_or_ecnl_port).map_err(|e| write_err("port", &e));
        ...
        thread::Builder::new().name(thread_name).spawn( move || {
            ...
            let simulated_port_or_ecnl_port = {
                let simulated_port = simulated_port_or_ecnl_port.left().expect("ecnl in simulator");
                Either::Left(simulated_port)
            };
            #[cfg(feature = "cell")]
            let simulated_port_or_ecnl_port: Either<SimulatedPort, Arc<ECNL_Port>> = {
                Either::Right(simulated_port_or_ecnl_port.right().unwrap())
            };
    ...
    fn listen_link_loop(&mut self, simulated_port_or_ecnl_port: &Either<SimulatedPort, Arc<ECNL_Port>>) -> Result<(), Error> {
        ...
        #[cfg(any(feature = "noc", feature = "simulator"))] {
            let simulated_port = simulated_port_or_ecnl_port.left().expect("ecnl in simulator");
	    return simulated_port.listen(self, port_to_pe);
        }
        #[cfg(feature = "cell")] {
            let ecnl_port = simulated_port_or_ecnl_port.right().expect("port_link_channel in cell");
    ...
    fn listen_pe_loop(&self, simulated_port_or_ecnl_port: &Either<SimulatedPort, Arc<ECNL_Port>>, port_from_pe: &PortFromPe) -> Result<(), Error> {
        ...
        loop {
            ...
            #[cfg(any(feature = "noc", feature = "simulator"))]
		simulated_port_or_ecnl_port.left().expect("ecnl port in simulator").send(packet)?;
            #[cfg(feature = "cell")]
            {
                simulated_port_or_ecnl_port.right().expect("simulated port in cell").send(&packet)?;
            }
        }
    }
