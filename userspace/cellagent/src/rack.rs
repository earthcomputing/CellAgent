...
use crate::ec_message_formats::{PortFromLink, PortToLink, LinkToPort, LinkFromPort, PortFromPe};
use crate::link::{Link};
...
impl Rack {
    ...
    pub fn initialize(&mut self, blueprint: &Blueprint)  -> Result<Vec<JoinHandle<()>>, Error> {
        ...
        for edge in edge_list {
            ...
            left_port.link_channel(Either::Left(SimulatedPort::new(&left_port,left_to_link, left_from_link, left_from_pe.clone())), left_from_pe);
            rite_port.link_channel(Either::Left(SimulatedPort::new(&rite_port, rite_to_link, rite_from_link, rite_from_pe.clone())), rite_from_pe);
