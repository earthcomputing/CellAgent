...
use crate::config::{CellQty, MaskValue, PortQty, CONFIG, MASK_MAX, PAYLOAD_DEFAULT_ELEMENT, REPO};
use crate::name::TreeID;
use crate::uuid_ec::Uuid;

pub const BASE_TENANT_MASK: Mask = Mask {
    mask: MaskValue(MASK_MAX),
}; // All ports
pub const DEFAULT_USER_MASK: Mask = Mask {
    mask: MaskValue(MASK_MAX - 1),
}; // All ports except port 0
...
impl Mask {
    ...
    pub fn empty() -> Mask {
        Mask { mask: MaskValue(0) }
    }
    pub fn all_but_zero(no_ports: PortQty) -> Mask {
        Mask {
            mask: MaskValue((2 as u16).pow((*no_ports) as u32) - 2),
        }
    }
    pub fn _equal(self, other: Mask) -> bool {
        *self.mask == *other.mask
    }
    //pub fn get_as_value(&self) -> MaskValue { self.mask }
    pub fn or(self, mask: Mask) -> Mask {
        Mask {
            mask: MaskValue(*self.mask | *mask.mask),
        }
    }
    ...
    pub fn get_port_nos(self) -> Vec<PortNo> {
        (0..=*CONFIG.max_num_phys_ports_per_cell)
            .map(|i| {
                PortNo(i)
                    .make_port_number(CONFIG.max_num_phys_ports_per_cell)
...
impl Default for PortNo {
    fn default() -> Self {
        PortNo(0)
    }
}
impl Deref for PortNo {
    type Target = u8;
...
impl PortNumber {
    pub fn new(no: PortNo, no_ports: PortQty) -> Result<PortNumber, Error> {
        if *no > *no_ports {
            Err(UtilityError::PortNumber {
                port_no: no,
                func_name: "PortNumber::new",
                max: no_ports,
            }
            .into())
        } else {
            Ok(PortNumber {
                port_no: (no as PortNo),
            })
        }
    }
    pub fn new0() -> PortNumber {
        PortNumber {
            port_no: (PortNo(0)),
        }
    }
    ...
    pub fn as_usize(self) -> usize {
        *self.port_no as usize
    }
}
impl fmt::Display for PortNumber {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", *self.port_no)
    }
}
...
impl Path {
    ...
    pub fn new0() -> Path {
        Path {
            port_number: PortNumber::new0(),
        }
    }
    pub fn get_port_number(self) -> PortNumber {
        self.port_number
    }
    pub fn get_port_no(self) -> PortNo {
        self.port_number.get_port_no()
    }
}
...
impl<T: Sized> Stack<T> {
    ...
    pub fn _push(&mut self, element: T) {
...
impl TraceHeader {
    ...
    pub fn spawning_thread_id(&self) -> u64 {
        self.spawning_thread_id
    }
    pub fn thread_id(&self) -> u64 {
        self.thread_id
    }
    pub fn event_id(&self) -> &Vec<u64> {
        &self.event_id
    }
    ...
    pub fn get_event_id(&self) -> Vec<u64> {
        self.event_id.clone()
    }
    pub fn parse(thread_id: ThreadId) -> u64 {
        ...
        n_as_str[0]
            .parse()
            .expect(&format!("Problem parsing ThreadId {:?}", thread_id))
    }
}
fn timestamp() -> u64 {
...
impl TraceHeaderParams {
    pub fn get_module(&self) -> &'static str {
        self.module
    }
...
pub struct CellInfo {
    // Any data the cell agent wants to expose to applications
...
impl fmt::Display for ByteArray {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let bytes =
            std::str::from_utf8(&self.bytes).expect("ByteArray: Error converting bytes to str");
        write!(f, "{}", bytes)
    }
}
...
pub struct ActivityData {
    // Currently is Copy, but I don't want to depend on that
    data: usize,
}
impl ActivityData {
...
impl Default for ActivityData {
    // Used to initialize and to increment
    fn default() -> Self {
        ActivityData { data: 1 }
    }
}
...
impl fmt::Display for HolSelector {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "Head of Line Selecter: Trees to block {:?}",
            self.selector
        )
    }
}
...
// Utility functions
pub fn is2e(i: usize, j: usize) -> Edge {
    Edge(CellNo(i), CellNo(j))
}
pub fn _print_vec<T: fmt::Display>(vector: &[T]) {
    for (count, v) in vector.iter().enumerate() {
        println!("{:3}: {}", count, v)
    }
}
...
pub fn print_hash_map<
    K: fmt::Display + Eq + std::hash::Hash,
...
pub fn write_err(caller: &str, e: &Error) {
    ...
    if fail.cause().and_then(|cause| cause.backtrace()).is_some() {
        let _ = writeln!(
            stderr,
            "---> Backtrace available: uncomment line in utility.rs containing --->"
        );
...
pub fn _string_to_object(string: &str) -> Result<Value, Error> {
    ...
    let v = serde_json::from_str(string).context(UtilityError::Chain {
        func_name: _f,
        comment: S(""),
    })?;
    Ok(v)
...
pub fn _dbg_get_thread_id() -> u64 {
    TraceHeader::parse(thread::current().id())
}
...
pub fn S<T: fmt::Display>(s: T) -> String {
    s.to_string()
}
// Errors
...
pub enum UtilityError {
    ...
    #[fail(
        display = "UtilityError::PortNumber {}: Port number {:?} is larger than the maximum of {:?}",
        func_name, port_no, max
    )]
    PortNumber {
        port_no: PortNo,
        func_name: &'static str,
    ...
    #[fail(
        display = "UtilityError::Unimplemented {}: {} is not implemented",
        func_name, feature
    )]
    Unimplemented {
        feature: String,
