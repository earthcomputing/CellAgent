...
extern ecnl_port_t port_create(uint8_t port_id) {
    ...
    ecnl_port_t ecnl_port;
    ecnl_port.port_sock = sock;
    ecnl_port.port_esock = esock;
    ecnl_port.port_module_id = 0; // hardwired
    ecnl_port.port_id = port_id;

    link_state_t link_state; 
    get_link_state(&ecnl_port, &link_state);
    ecnl_port.port_up_down = link_state.port_link_state;
    ecnl_port.port_name = link_state.port_name; // fill in name
    return ecnl_port;
}
