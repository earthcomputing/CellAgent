...
impl<'a> Datacenter<'a> {
	pub fn new(scope: &Scope, ncells: CellNo, nports: PortNo, edge_list: Vec<(CellNo,CellNo)>) -> 
		...
		if ncells < 2  {
			...
			return Err(ErrorKind::CellsSizeError(ncells).into());
		}
		if edge_list.len() < ncells - 1 {
			println!("nlinks {}", edge_list.len());
			return Err(ErrorKind::LinksSizeError(edge_list.len()).into());			
		}
		...
		for edge in edge_list {
			if edge.0 == edge.1 { return Err(ErrorKind::WireError(edge).into()); }
			if (edge.0 > ncells) | (edge.1 >= ncells) { return Err(ErrorKind::WireError(edge).into()); }
			let split = cells.split_at_mut(max(edge.0,edge.1));
			let mut cell = match split.0.get_mut(edge.0) {
				Some(c) => c,
				None => return Err(ErrorKind::WireError(edge).into())
