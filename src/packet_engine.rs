...
impl PacketEngine {
    ...
    fn listen_port(&self, pe_from_ports: PeFromPort, pe_from_pe: PeFromPe,
                   outer_trace_header: TraceHeader)
            -> Result<(),Error> {
        let _f = "listen_port";
        let mut pe = self.clone();
        let mut outer_trace_header_clone = outer_trace_header.clone();
        ...
        loop {
            ...
            match msg {
                ...
                CmToPePacket::Packet((user_mask, packet)) => {
                    self.route_packet(user_mask, packet, trace_header)?;
                }
            };
        }
    }

    fn route_packet(&mut self, user_mask: Mask, mut packet: Packet, trace_header: &mut TraceHeader) -> Result<(), Error> {
        let _f = "route_packet";
    ...
    fn listen_port_loop(&mut self, pe_from_ports: &PeFromPort, pe_from_pe: &PeFromPe,
        ...
        loop {
            ...
            {
                let ref trace_params = TraceHeaderParams { module: file!(), line_no: line!(), function: _f, format: "pl_recv" };
                let trace = json!({ "cell_id": &self.cell_id, "msg": &msg.clone() });
                let _ = dal::add_to_trace(trace_header, TraceType::Trace, trace_params, &trace, _f);
            }
            match msg {
        ...
        match packet.get_ait_state() {
            ...
            AitState::Tock => {
                ...
                self.pe_to_cm.send(PeToCmPacket::Packet((port_no, packet))).or_else(|_| -> Result<(), Error> {
                    ...
                    packet.make_tock();
                    packet.time_reverse();
                    Ok(channel.send(PeToPortPacket::Packet(packet))?)
                })?;
            },
