...
//	pub fn get_id(&self) -> &LinkID { &self.id }
	pub fn start_threads(&self, 
		...
		let left_handle = self.listen(link_to_left.clone(), link_from_left, link_to_rite.clone()).context(LinkError::Chain { func_name: "start_threads", comment: S(self.id.clone()) + " left"})?;
		let rite_handle = self.listen(link_to_rite, link_from_rite, link_to_left).context(LinkError::Chain { func_name: "start_threads", comment: S(self.id.clone()) + " rite"})?;
