...
pub enum CaToCmBytes { Entry(RoutingTableEntry), Bytes(CATOCM), Reroute(REROUTE), 
    Status((PortNo, bool, NumberOfPackets, PortStatus)), Tunnel(TUNNEL) }
...
#[derive(Debug, Clone, Serialize)]
pub enum CmToPePacket { Entry(RoutingTableEntry), Packet((Mask, Packet)), Reroute(REROUTE) }
...
#[derive(Debug, Clone, Serialize)]
pub enum PeToCmPacket { Status((PortNo, bool, NumberOfPackets, PortStatus)), Packet((PortNo, Packet)) }
...
// Cmodel to CellAgent
#[derive(Debug, Clone, Serialize)]
pub enum CmToCaBytes { Status((PortNo, bool, NumberOfPackets, PortStatus)),
    Bytes((PortNo, bool, Uuid, ByteArray)), Tunnel(TUNNEL) }
