...
use crate::blueprint::BorderCell;
...
fn main() -> Result<(), Error> {
    ...
    let port_list : Vec<PortNo> = (0..*num_phys_ports as usize)
    ...
    let border_port_list: Vec<PortNo> = vec![0u8]
        ...
        .map(|i| PortNo(*i as u8))
	.collect();
    let border: HashSet<PortNo> = HashSet::from_iter(border_port_list.clone());
    ...
    let cell = BorderCell::new(cell_no, CellType::Border, interior_port_list, border_port_list);
    let nal_cell = NalCell::new(cell_no, num_phys_ports, &HashSet::from_iter(cell.get_border_ports().clone()), CellType::Border, CellConfig::Large);
    Ok(())
}
