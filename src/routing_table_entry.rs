...
impl RoutingTableEntry {
	...
	pub fn default(index: TableIndex) -> Result<RoutingTableEntry> {
		...
		Ok(RoutingTableEntry::new(index, TreeID::new("Default")?, false, port_number, Mask::empty(), [TableIndex{v:0}; MAX_PORTS.v as usize]))
	}
...
impl fmt::Display for RoutingTableEntry {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result { 
		let mut s = format!("{:6}", self.index.v);
		...
		for other_index in self.other_indices.iter() {
			other_indices.push(other_index.v);
		}
...
error_chain! {
	...
	errors { RoutingTableEntryError
		Index(index: TableIndex) {
			...
			display("Index number {} is greater than the maximum of {}", index.v, MAX_ENTRIES.v)
		}
	}
}
