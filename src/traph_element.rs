...
impl TraphElement {
    ...
    pub fn default(port_number: PortNumber) -> TraphElement {
        ...
        TraphElement::new(false, port_no, PortState::Pruned,
    ...
    pub fn get_status(&self) -> PortState { self.status }
    pub fn is_status(&self, status: PortState) -> bool { self.status == status }
...
//  pub fn set_disconnected(&mut self) { self.is_connected = false; }
    fn set_status(&mut self, status: PortState) { self.status = status; }
    pub fn is_on_broken_path(&self, broken_path: Path) -> bool { self.path == broken_path }
    pub fn mark_parent(&mut self) { self.set_status(PortState::Parent) }
    pub fn mark_child(&mut self)  { self.set_status(PortState::Child) }
    pub fn mark_pruned(&mut self) { self.set_status(PortState::Pruned) }
    pub fn mark_broken(&mut self) { self.set_status(PortState::Broken) }
}
