...
impl CellAgent {
	pub fn new(cell_id: &CellID, cell_type: CellType, no_ports: PortNo, ca_to_pe: CaToPe ) 
		...
		let my_tree_id = TreeID::new(cell_id.get_name()).chain_err(|| ErrorKind::TreeName(cell_id.clone(), S(f), cell_id.get_name().to_string()))?;
		my_tree_id.append2file().chain_err(|| ErrorKind::Trace(cell_id.clone(), S(f)))?;		
		let control_tree_id = TreeID::new(&(cell_id.get_name().to_string() + ":" + CONTROL_TREE_NAME)).chain_err(|| ErrorKind::TreeName(cell_id.clone(), S(f), CONTROL_TREE_NAME.to_string()))?;
		let connected_tree_id = TreeID::new(&(cell_id.get_name().to_string() + ":" + CONNECTED_PORTS_TREE_NAME)).chain_err(|| ErrorKind::TreeName(cell_id.clone(), S(f), CONNECTED_PORTS_TREE_NAME.to_string()))?;
		connected_tree_id.append2file().chain_err(|| ErrorKind::Trace(cell_id.clone(), S(f)))?;
		...
		Ok(CellAgent { cell_id: cell_id.clone(), my_tree_id: my_tree_id, cell_type: cell_type,
			...
			my_entry: RoutingTableEntry::default(TableIndex(0)).chain_err(|| ErrorKind::RoutingTableEntry(cell_id.clone(), S(f)))?, 
			connected_tree_entry: Arc::new(Mutex::new(RoutingTableEntry::default(TableIndex(0)).chain_err(|| ErrorKind::RoutingTableEntry(cell_id.clone(), S("new")))?)),
	...
	pub fn initialize(&mut self, cell_type: CellType, ca_from_pe: CaFromPe) -> Result<()> {
		...
		self.update_traph(&control_tree_id, port_number_0, 
				...
				&mut HashSet::new(), other_index, hops, path).chain_err(|| ErrorKind::UpdateTraph(self.cell_id.clone(), S(f), control_tree_id))?;
		let mut eqns = HashSet::new();
		...
		let connected_tree_entry = self.update_traph(&connected_tree_id, port_number_0, 
			traph::PortStatus::Parent, Some(gvm_equation),
			&mut HashSet::new(), other_index, hops, path).chain_err(|| ErrorKind::UpdateTraph(self.cell_id.clone(), S(f), connected_tree_id))?;
		...
		self.my_entry = self.update_traph(&my_tree_id, port_number_0, 
				traph::PortStatus::Parent, Some(gvm_equation), 
				&mut HashSet::new(), other_index, hops, path).chain_err(|| ErrorKind::UpdateTraph(self.cell_id.clone(), S(f), my_tree_id))?; 
		self.listen(ca_from_pe).chain_err(|| ErrorKind::ListenPe(self.cell_id.clone(), S(f)))?;
		Ok(())
	}
	...
	pub fn get_hops(&self, tree_id: &TreeID) -> Result<PathLength> {
		...
		if let Some(traph) = self.traphs.lock().unwrap().get(&tree_id.get_uuid()) {
			Ok(traph.get_hops().chain_err(|| ErrorKind::Traph(self.cell_id.clone(), S(f), tree_id.clone()))?)
		} else {
			Err(ErrorKind::Tree(self.cell_id.clone(), S("get_hops"), tree_id.get_uuid()).into())
		}
	}
	...
	pub fn add_saved_msg(&mut self, packets: &SavedMsg) -> Vec<SavedMsg> {
		...
		{ 
			...
			let msg = MsgType::get_msg(&packets).chain_err(|| ErrorKind::MessageAssembly(self.cell_id.clone(), S(f))).unwrap();
	...
	pub fn create_vms(&mut self, service_sets: Vec<Vec<Service>>) -> Result<()> {
		...
		let up_traph_id = UpTraphID::new(&name).chain_err(|| ErrorKind::Name(self.cell_id.clone(), S(f), name))?;
		let mut ca_to_vms = Vec::new();
		let (vm_to_ca, ca_from_vm): (VmToCa, CaFromVm) = channel();
		let ca_id = TreeID::new("CellAgent").chain_err(|| ErrorKind::Name(self.cell_id.clone(), S(f), "CellAgent".to_string()))?;
		let base_id = TreeID::new("BaseTree").chain_err(|| ErrorKind::Name(self.cell_id.clone(), S(f), "BaseTree".to_string()))?;
		...
		for mut services in service_sets {
			...
			let vm_id = VmID::new(&self.cell_id, self.vm_id_no).chain_err(|| ErrorKind::Name(self.cell_id.clone(), S(f), self.cell_id.get_name().to_string() + &self.vm_id_no.to_string()))?;
			...
			vm.initialize(&mut services, &up_traph_id, &tree_ids,
				&vm_to_ca, vm_from_ca).chain_err(|| ErrorKind::VM(self.cell_id.clone(), S(f), vm_id))?;
			ca_to_vms.push(ca_to_vm);
		}
		self.up_traphs_senders.insert(up_traph_id.clone(), ca_to_vms);
		self.listen_uptraph(up_traph_id.clone(), ca_from_vm).chain_err(|| ErrorKind::ListenUpTraph(self.cell_id.clone(), S(f), up_traph_id))?;
		Ok(())
	}
	fn listen_uptraph(&self, up_traph_id: UpTraphID, ca_from_vm: CaFromVm) -> Result<()> {
		...
		loop {
			let msg = ca_from_vm.recv().chain_err(|| ErrorKind::ListenUpTraph(ca.cell_id.clone(), S(f), up_traph_id.clone()))?;
		...
		let entry = {
			...
			let mut traph = match traphs.entry(black_tree_id.get_uuid()) { // Avoids lifetime problem
				...
				Entry::Vacant(v) => {
					...
					let index = self.clone().use_index().chain_err(|| ErrorKind::Index(self.cell_id.clone(), S(f)))?;
					let t = Traph::new(&self.cell_id, &black_tree_id, index).chain_err(|| ErrorKind::Traph(self.cell_id.clone(), S(f), black_tree_id.clone()))?;
					v.insert(t)
				}
			};
			let (gvm_recv, gvm_send, gvm_xtnd, gvm_save) = match gvm_equation {
				Some(eqn) => {
					let variables = traph.get_params(eqn.get_variables()).chain_err(|| ErrorKind::Traph(self.cell_id.clone(), S(f), black_tree_id.clone()))?;
					let recv = eqn.eval_recv(&variables).chain_err(|| ErrorKind::Gvm(self.cell_id.clone(), S(f), eqn.clone()))?;
					let send = eqn.eval_send(&variables).chain_err(|| ErrorKind::Gvm(self.cell_id.clone(), S(f), eqn.clone()))?;
					let xtnd = eqn.eval_xtnd(&variables).chain_err(|| ErrorKind::Gvm(self.cell_id.clone(), S(f), eqn.clone()))?; 
					let save = eqn.eval_save(&variables).chain_err(|| ErrorKind::Gvm(self.cell_id.clone(), S(f), eqn.clone()))?; 
					(recv, send, xtnd, save)
				},
			...
			let (hops, path) = match port_status {
				traph::PortStatus::Child => {
					let element = traph.get_parent_element().chain_err(|| ErrorKind::Traph(self.cell_id.clone(), S(f), black_tree_id.clone()))?;
			...
			let mut entry = traph.new_element(black_tree_id, port_number, port_status, other_index, children, hops, path).chain_err(|| ErrorKind::Traph(self.cell_id.clone(), S(f), black_tree_id.clone()))?; 
			...
			self.ca_to_pe.send(CaToPePacket::Entry(entry)).chain_err(|| ErrorKind::SendPe(self.cell_id.clone(), S(f)))?;
			// Update entries for stacked trees
			let entries = traph.update_stacked_entries(entry).chain_err(|| ErrorKind::Traph(self.cell_id.clone(), S(f), black_tree_id.clone()))?;
	...
	pub fn stack_tree(&mut self, tree_id: &TreeID, parent_tree_uuid: &Uuid, 
		...
		let f = "stack_tree";
		let mut traph = self.get_traph(black_tree_id).chain_err(|| ErrorKind::Traph(self.cell_id.clone(), S(f), black_tree_id.clone()))?;
		if traph.has_tree(tree_id) { return Ok(()); } // Check for redundant StackTreeMsg
		let parent_entry = traph.get_tree_entry(&parent_tree_uuid).chain_err(|| ErrorKind::TraphEntry(self.cell_id.clone(), S(f), parent_tree_uuid.clone()))?;
		let mut entry = parent_entry.clone();
		let index = self.use_index()?; 
		entry.set_table_index(index);
		entry.set_uuid(&tree_id.get_uuid());
		let params = traph.get_params(gvm_eqn.get_variables()).chain_err(|| ErrorKind::Traph(self.cell_id.clone(), S(f), tree_id.clone()))?;
		if !gvm_eqn.eval_recv(&params).chain_err(|| ErrorKind::Gvm(self.cell_id.clone(), S(f), gvm_eqn.clone()))? { 
			let mask = entry.get_mask().and(Mask::all_but_zero(self.no_ports));
			entry.set_mask(mask);
		}
		if !gvm_eqn.eval_xtnd(&params).chain_err(|| ErrorKind::Gvm(self.cell_id.clone(), S(f), gvm_eqn.clone()))? { entry.clear_children(); }
		if gvm_eqn.eval_send(&params).chain_err(|| ErrorKind::Gvm(self.cell_id.clone(), S(f), gvm_eqn.clone()))? { entry.enable_send(); } else { entry.disable_send(); }
	...
	fn listen(&mut self, ca_from_pe: CaFromPe) -> Result<()>{
		...
		::std::thread::spawn( move || { 
			let _ = ca.listen_loop(ca_from_pe).chain_err(|| ErrorKind::ListenPe(ca.cell_id.clone(), S(f))).map_err(|e| ca.write_err(e));
		});
		Ok(())
	}
	fn listen_loop(&mut self, ca_from_pe: CaFromPe) -> Result<()> {
		...
		loop {
			...
			match ca_from_pe.recv().chain_err(|| ErrorKind::ListenPe(self.cell_id.clone(), S(f)))? {
				PeToCaPacket::Status(port_no, is_border, status) => match status {
					port::PortStatus::Connected => self.port_connected(port_no, is_border).chain_err(|| ErrorKind::Port(self.cell_id.clone(), S(f), port_no))?,
					port::PortStatus::Disconnected => self.port_disconnected(port_no).chain_err(|| ErrorKind::Port(self.cell_id.clone(), S(f), port_no))?
				},
				PeToCaPacket::Packet(port_no, packet) => {
					...
					if last_packet {
						let mut msg = MsgType::get_msg(&packets).chain_err(|| ErrorKind::MessageAssembly(self.cell_id.clone(), S(f)))?;
						msg.process(self, tree_uuid, port_no).chain_err(|| ErrorKind::Message(self.cell_id.clone(), S(f), msg.get_count()))?;
	...
	fn gvm_eval_save(&self, tree_uuid: Uuid, msg: Box<Message>) -> Result<bool> {
		...
		if let Some(gvm_eqn) = msg.get_payload().get_gvm_eqn() {
			...
			if let Some(black_tree_uuid) = self.tree_map.lock().unwrap().get(&tree_uuid) {
				...
				let params = traph.get_params(gvm_eqn.get_variables()).chain_err(|| ErrorKind::TraphEntry(self.cell_id.clone(), S(f), tree_uuid))?;
				let save = gvm_eqn.eval_save(&params).chain_err(|| ErrorKind::Gvm(self.cell_id.clone(), S(f), gvm_eqn.clone()))?;
				Ok(save)
			} else {
	...
	fn port_connected(&mut self, port_no: PortNo, is_border: bool) -> Result<()> {
		...
		if is_border {
			...
			let tree_id = self.my_tree_id.add_component("Outside").chain_err(|| ErrorKind::Name(self.cell_id.clone(), S(f), S("Outside")))?;
			let tree_id = TreeID::new(tree_id.get_name()).chain_err(|| ErrorKind::Name(self.cell_id.clone(), S(f), tree_id.to_string()))?;
			tree_id.append2file().chain_err(|| ErrorKind::Trace(self.cell_id.clone(), S(f)))?;
			let ref my_tree_id = self.my_tree_id.clone(); // Need because self borrowed mut
			let msg = StackTreeMsg::new(&tree_id, &self.my_tree_id).chain_err(|| ErrorKind::MessageCreate(self.cell_id.clone(), S(f), S("StackTree")))?;
			let packets = msg.to_packets(&self.my_tree_id).chain_err(|| ErrorKind::Packetize(self.cell_id.clone(), S(f), msg.get_count()))?;
			let port_no_mask = Mask::all_but_zero(self.no_ports);
			let my_index = self.my_entry.get_index();
			for packet in packets {
				self.ca_to_pe.send(CaToPePacket::Packet((my_index, port_no_mask, packet))).chain_err(|| ErrorKind::SendPe(self.cell_id.clone(), S(f)))?;			
			}
			let mut eqns = HashSet::new();
			eqns.insert(GvmEqn::Recv("true"));
			...
			self.stack_tree(&tree_id, &my_tree_id.get_uuid(), my_tree_id, &gvm_eqn).chain_err(|| ErrorKind::StackTree(self.cell_id.clone(), S(f), tree_id.clone()))?;
		} else {
			//println!("Cell {}: port {} connected", self.cell_id, *port_no);
			let port_no_mask = Mask::new(PortNumber::new(port_no, self.no_ports).chain_err(|| ErrorKind::PortNumber(self.cell_id.clone(), S(f), port_no))?);
			let path = Path::new(port_no, self.no_ports).chain_err(|| ErrorKind::PortNumber(self.cell_id.clone(), S(f), port_no))?;
			...
			let packets = msg.to_packets(&self.control_tree_id).chain_err(|| ErrorKind::Packetize(self.cell_id.clone(), S(f), msg.get_count()))?;
			...
			for packet in packets {
				...
				self.ca_to_pe.send(entry).chain_err(|| ErrorKind::SendPe(self.cell_id.clone(), S(f)))?;
				let packet_msg = CaToPePacket::Packet((connected_tree_index, port_no_mask, packet));
				self.ca_to_pe.send(packet_msg).chain_err(|| ErrorKind::SendPe(self.cell_id.clone(), S(f)))?;
			}
			let saved_msgs  = self.get_saved_msgs();
			self.forward_discover(&saved_msgs, port_no_mask).chain_err(|| ErrorKind::Forward(self.cell_id.clone(), S(f)))?;		
		}
		Ok(())		
	}
	...
	pub fn forward_discover(&mut self, saved_msgs: &Vec<SavedMsg>, mask: Mask) -> Result<()> {
		...
		for packets in saved_msgs.iter() {
			self.send_msg(self.connected_tree_id.get_uuid(), packets, mask).chain_err(|| ErrorKind::Forward(self.cell_id.clone(), S(f)))?;
	...
	pub fn forward_saved(&mut self, saved_msgs: &Vec<SavedMsg>, mask: Mask) -> Result<()> {
		...
		for packets in saved_msgs.iter() {
			...
			self.send_msg(uuid, packets, mask).chain_err(|| ErrorKind::Forward(self.cell_id.clone(), S(f)))?;
			//println!("CellAgent {}: forward on ports {:?} {}", self.cell_id, mask.get_port_nos(), msg);
		}
		Ok(())	
	}
	pub fn send_msg(&self, tree_uuid: Uuid, packets: &Vec<Packet>, user_mask: Mask) 
		...
		let index = {
			if let Some(traph) = self.traphs.lock().unwrap().get(&tree_uuid) {
				traph.get_table_index(&tree_uuid).chain_err(|| ErrorKind::Index(self.cell_id.clone(), S(f)))?			
			} else {
		...
		for packet in packets.iter() {
			...
			self.ca_to_pe.send(msg).chain_err(|| ErrorKind::SendPe(self.cell_id.clone(), S(f)))?;
...
error_chain! {
	errors { 
		Forward(cell_id: CellID, func_name: String) {
			display("Cellagent {}: Problem forwarding discover messages on cell {}", func_name, cell_id)
		}
		Gvm(cell_id: CellID, func_name: String, gvm_eqn: GvmEquation) {
			display("Cellagent {}: Problem evaluating {} on cell {}", func_name, gvm_eqn, cell_id)
		}
		Index(cell_id: CellID, func_name: String) {
			display("Cellagent {}: Can't get routing table index on cell {}", func_name, cell_id)
		}
		ListenPe(cell_id: CellID, func_name: String) {
			display("Cellagent {}: Error listening to PacketEngine on cell {}", func_name, cell_id)
		}
		ListenUpTraph(cell_id: CellID, func_name: String, up_traph_id: UpTraphID) {
			display("Cellagent {}: Error listenig to VM {} on cell {}", func_name, up_traph_id, cell_id)
		}
		// Recursive type error if put in message.rs
		Message(cell_id: CellID, func_name: String, msg_id: MsgID) {
			display("Cellagent {}: Error processing message {} on cell {}", func_name, **msg_id, cell_id)
		}		
		MessageAssembly(cell_id: CellID, func_name: String) {
			display("Cellagent {}: Problem assembling message on cell {}", func_name, cell_id)
		}
		MessageCreate(cell_id: CellID, func_name: String, msg_type: String) {
			display("Cellagent {}: Cell {} could not create a {} message", func_name, cell_id, msg_type)
		}
		Name(cell_id: CellID, func_name: String, name: String) {
			display("Cellagent {}: Error making name from {} on cell {}", func_name, name, cell_id)
		}
		...
		Packetize(cell_id: CellID, func_name: String, msg_id: MsgID) {
			display("Cellagent {}: CellID {} could not packetize message {}", func_name, cell_id, **msg_id)
		}
		Port(cell_id: CellID, func_name: String, port_no: PortNo) {
			display("Cellagent {}: Problem with port {} on cell {}", func_name, port_no.v, cell_id)
		}
		PortNumber(cell_id: CellID, func_name: String, port_no: PortNo) {
			display("Cellagent {}: {} is not a valid port number on cell {}", func_name, port_no.v, cell_id)
		}
		PortTaken(cell_id: CellID, func_name: String, port_no: PortNo) {
			display("Cellagent {}: Receiver for port {} has been previously assigned on cell {}", func_name, port_no.v, cell_id)
		}
		Recvr(cell_id: CellID, func_name: String, port_no: PortNo) {
			display("Cellagent {}: No receiver for port {} on cell {}", func_name, port_no.v, cell_id)
		}
		RoutingTableEntry(cell_id: CellID, func_name: String) {
			display("Cellagent {}: Error creating default routing table entry on cell {}", func_name, cell_id)
		}
		SendPe(cell_id: CellID, func_name: String) {
			display("Cellagent {}: Error sending to PacketEngine on {}", func_name, cell_id)
		}
		...
		StackedTree(cell_id: CellID, func_name: String, tree_uuid: Uuid) {
			display("Cellagent {}: Cell {} has no black tree for stacked tree {}", func_name, cell_id, tree_uuid)
		}
		...
		Traph(cell_id: CellID, func_name: String, tree_id: TreeID) {
			display("Cellagent {}: Invalid traph for tree {} on cell {}", func_name, tree_id, cell_id)
		}
		TraphEntry(cell_id: CellID, func_name: String, uuid: Uuid) {
			display("Cellagent {}: No entry on cell {} for parent UUID {}", func_name, cell_id, uuid)
		}
		...
		TreeName(cell_id: CellID, func_name: String, tree_name: String) {
			display("Cellagent {}: Problem constructing TreeID from {} on cell {}", func_name, tree_name, cell_id) 
		}
		UnknownVariable(cell_id: CellID, func_name: String, var: GvmVariable) {
			display("Cellagent {}: Cell {}: Unknown gvm variable {}", func_name, cell_id, var)
		}
		UpdateTraph(cell_id: CellID, func_name: String, tree_id: TreeID) {
			display("Cellagent {}: Problem updating traph for tree {} on cell {}", func_name, tree_id, cell_id)
		}
		UpTraph(cell_id: CellID, func_name: String, up_tree_id: UpTraphID) {
			display("Cellagent {}: Cell {} has no UpTraph named {}", func_name, cell_id, up_tree_id)
		}
		VM(cell_id: CellID, func_name: String, vm_id: VmID) {
			display("Cellagent {}: Problem initializing VM {} on cell {}", func_name, vm_id, cell_id)
		}
	}
}
