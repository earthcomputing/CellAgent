...
pub struct CellAgent {
    ...
    failover_reply_port: HashMap<PortTreeID, PortNo>,
}
impl CellAgent {
...
//    pub fn get_tree_name_map(&self) -> &TreeNameMap { &self.tree_name_map }
    ...
    fn get_mask(&self, port_tree_id: &PortTreeID) -> Result<Mask, Error> {
        let _f = "get_mask";
        Ok(match self.get_traph_clone(port_tree_id) {
            Ok(t) => {
    ...
    fn get_gvm_eqn(&self, port_tree_id: &PortTreeID) -> Result<GvmEquation, Error> {
        ...
        let traph = self.get_traph_clone(port_tree_id).context(CellagentError::Chain { func_name: _f, comment: S("") })?;
    ...
    fn get_base_tree_id(&self, port_tree_id: &PortTreeID) -> Result<TreeID, Error> {
        ...
        self.base_tree_map
            .get(&port_tree_id)
            .cloned()
            .ok_or(CellagentError::BaseTree { func_name: _f, cell_id: self.cell_id.clone(), tree_id: port_tree_id.clone() }.into())
    }
    ...
    fn update_traph(&mut self, base_tree_id: &TreeID, port_number: PortNumber, port_status: traph::PortStatus,
        ...
        let port_tree_id = base_tree_id.to_port_tree_id(port_number);
    ...
    fn get_traph_mut<'a>(&self, locked: &'a mut MutexGuard<Traphs>, port_tree_id: &PortTreeID) -> Result<&'a mut Traph, Error> {
    ...
    fn get_traph_clone(&self, port_tree_id: &PortTreeID) -> Result<Traph, Error> {
        let _f = "get_traph_clone";
        let base_tree_id = self.get_base_tree_id(port_tree_id).context(CellagentError::Chain { func_name: _f, comment: S("") })?;
        let uuid = base_tree_id.get_uuid();
        self.traphs.lock().unwrap()
            .get(&uuid)
    ...
    fn own_traph(&mut self, port_tree_id: &PortTreeID) -> Result<Traph, Error> {
        let _f = "own_traph";
        let base_tree_id = self.get_base_tree_id(port_tree_id).context(CellagentError::Chain { func_name: _f, comment: S("") })?;
        let uuid = base_tree_id.get_uuid();
        self.traphs.lock().unwrap()
            .remove(&uuid)
            .ok_or(CellagentError::NoTraph { cell_id: self.cell_id.clone(), func_name: "stack_tree", tree_id: base_tree_id }.into())
    }
    fn insert_traph(&mut self, port_tree_id: &PortTreeID, traph: Traph) -> Result<(), Error> {
        let _f = "insert_traphs";
        let base_tree_id = self.get_base_tree_id(port_tree_id).context(CellagentError::Chain { func_name: _f, comment: S("") })?;
        let uuid = base_tree_id.get_uuid();
    ...
    fn deploy(&mut self, sender_id: &SenderID, deployment_port_tree_id: &PortTreeID, _msg_tree_id: &PortTreeID,
        ...
        let mut tree_map = self.tree_name_map.lock().unwrap()
            .get(sender_id)
            .cloned()
            .ok_or::<Error>(CellagentError::TreeNameMap { cell_id: self.get_id(), func_name: _f, sender_id: sender_id.clone() }.into())?;
        ...
        for vm_spec in manifest.get_vms() {
            ...
            for vm_allowed_tree in vm_allowed_trees {
                tree_map.get(vm_allowed_tree.get_name())
                    .ok_or::<Error>(CellagentError::TreeMap { cell_id: self.cell_id.clone(), func_name: "deploy(vm)", tree_name: vm_allowed_tree.clone() }.into())
                    .map(|allowed_tree_id| {
    ...
    fn listen_uptree_loop(&mut self, sender_id: &SenderID, _vm_id: &VmID, ca_from_vm: &CaFromVm)
        ...
        loop {
            ...
            let tree_map = self.tree_name_map.lock().unwrap()
                .get(sender_id)
                .cloned()
                .ok_or::<Error>(CellagentError::TreeNameMap { func_name: _f, cell_id: self.cell_id.clone(), sender_id: sender_id.clone() }.into())?;
    ...
    */
    ...
    fn stack_tree(&mut self, sender_id: &SenderID, allowed_tree: &AllowedTree,
        ...
        let locked = &mut self.traphs.lock().unwrap();
        let traph = self.get_traph_mut(locked, &parent_tree_id).context(CellagentError::Chain { func_name: "stack_tree", comment: S("own_traph")})?;
            ...
            match msg {
                ...
                CmToCaBytes::Tcp((port_no, (is_ait, allowed_tree, msg_type, direction, bytes))) => {
                    ...
                    let sender_id = self.border_port_tree_id_map
                        .get(&port_number)
                        .cloned()
                        .ok_or::<Error>(CellagentError::Border { func_name: _f, cell_id: self.cell_id.clone(), port_no: *port_no }.into())?
                        .0;
                    let tree_map= &(self.tree_name_map.lock().unwrap()
                        .get(&sender_id)
                        .cloned()
    ...
    pub fn process_discover_d_msg(&mut self, msg: &DiscoverDMsg, port_no: PortNo)
        ...
        let _ = self.update_traph(&base_tree_id.to_tree_id(), port_number, traph::PortStatus::Child, &gvm_eqn,
    ...
    pub fn process_failover_msg(&mut self, msg: &FailoverMsg, port_no: PortNo) -> Result<(), Error> {
        ...
        if rw_tree_id == my_tree_id {
            ...
            let locked = &mut self.traphs.lock().unwrap();
            let my_traph = self.get_traph_mut(locked, &self.my_tree_id.to_port_tree_id_0()).context(CellagentError::Chain { func_name: _f, comment: S("my_traph") })?;
            let lw_traph = self.get_traph_clone(&lw_port_tree_id).context(CellagentError::Chain { func_name: _f, comment: S("lw_traph") })?;
            ...
            let changed_entries = my_traph.swap_child(&rw_port_tree_id, broken_port_number, port_number).context(CellagentError::Chain { func_name: _f, comment: S("swap child") })?;
    ...
    pub fn process_failover_d_msg(&mut self, msg: &FailoverDMsg, port_no: PortNo)
        ...
        match payload.get_response() {
            FailoverResponse::Success => {
                let rw_tree_id = rw_port_tree_id.to_tree_id();
                let mut rw_traph = self.own_traph(&rw_port_tree_id).context(CellagentError::Chain { func_name: _f, comment: S("") })?;
                let hops = rw_traph.get_parent_element()?.get_hops();
                let old_parent_entry = rw_traph.get_tree_entry(&rw_tree_id.get_uuid()).context(CellagentError::Chain { func_name: _f, comment: S("") })?;
                println!("Cellagent {}: {} rw tree id {} old entry {}", self.cell_id, _f, rw_tree_id, old_parent_entry);
                let old_parent = old_parent_entry.get_parent();
                self.failover_reply_port
                    .get(rw_port_tree_id)
                    .map_or_else(|| Ok(println!("---> Cellagent {}: {} reached leafward node for port tree {}", self.cell_id, _f, rw_port_tree_id)),
                        |failover_port| -> Result<(), Error> {
                        let children = [failover_port.make_port_number(self.no_ports)?].iter().cloned().collect::<HashSet<PortNumber>>();
                        let mut updated_entry = rw_traph.update_element(&rw_tree_id, port_number,
                                                                     traph::PortStatus::Parent, &children, hops, broken_path).context(CellagentError::Chain { func_name: _f, comment: S("") })?;
    ...
    pub fn process_manifest_msg(&mut self, msg: &ManifestMsg, port_no: PortNo, msg_port_tree_id: &PortTreeID)
        ...
        let traph = self.get_traph_clone(tree_id).context(CellagentError::Chain { func_name: _f, comment: S("") })?;
        ...
        let save = self.gvm_eval_save(msg_port_tree_id, &gvm_eqn).context(CellagentError::Chain { func_name: _f, comment: S(self.cell_id.clone())})?;
        if DEBUG_OPTIONS.trace_all || DEBUG_OPTIONS.process_msg {   // Debug;
    ...
    pub fn process_stack_tree_msg(&mut self, msg: &StackTreeMsg, port_no: PortNo, msg_port_tree_id: &PortTreeID)
        ...
        if let Some(entry) = self.stack_tree(sender_id, allowed_tree, new_port_tree_id, parent_tree_id,
            ...
            let mut traph = self.own_traph(new_port_tree_id)?;
            traph.set_tree_entry(&new_port_tree_id.get_uuid(), entry)?;
            self.insert_traph(new_port_tree_id, traph)?;
    ...
    pub fn process_stack_tree_d_msg(&mut self, msg: &StackTreeDMsg, port_no: PortNo) -> Result<(), Error> {
        ...
        let port_tree_id = msg.get_port_tree_id();
        let tree_uuid = port_tree_id.get_uuid();
        let mut traph = self.own_traph(&port_tree_id)?;
        ...
        self.insert_traph(port_tree_id, traph)?;
    ...
    fn find_new_parent(&mut self, header: &MsgHeader, payload: &FailoverMsgPayload, port_no: PortNo)
        ...
        match rw_traph.find_new_parent_port(&rw_port_tree_id, broken_path) {
            None => {
                println!("Cellagent {}: {} Failover failure for rootward port tree {}", self.cell_id, _f, rw_port_tree_id);
    ...
    fn tcp_application(&mut self, sender_id: &SenderID, _is_ait: bool, allowed_tree: &AllowedTree, serialized: &str,
        ...
        let tree_id = tree_map
            ...
            .ok_or::<Error>(CellagentError::TreeMap { func_name: _f, cell_id: self.cell_id.clone(), tree_name: allowed_tree.clone() }.into())?;
        let port_tree_id = tree_id.to_port_tree_id_0();
        if !self.may_send(&port_tree_id)? { return Err(CellagentError::MayNotSend { func_name: _f, cell_id: self.cell_id.clone(), tree_id: tree_id.clone() }.into()); }
    ...
    fn tcp_manifest(&mut self, sender_id: &SenderID, serialized: &str, _direction: MsgDirection,
        ...
        let _f = "tcp_manifest";
        let tcp_msg = &mut serde_json::from_str::<HashMap<String, String>>(&serialized).context(CellagentError::Chain { func_name: _f, comment: S(self.cell_id.clone()) + " deserialize StackTree" })?;
        let deploy_tree_name = &self.get_msg_params(tcp_msg, "deploy_tree_name").context(CellagentError::Chain { func_name: _f, comment: S(self.cell_id.clone()) + " parent tree name" })?;
        let deploy_tree_id = tree_map
            .get(AllowedTree::new(deploy_tree_name).get_name())
            .ok_or::<Error>(CellagentError::TreeMap { func_name: "listen_cm_loop 4", cell_id: self.cell_id.clone(), tree_name: AllowedTree::new(deploy_tree_name) }.into())?;
        let deploy_port_tree_id = &deploy_tree_id.to_port_tree_id_0();
        if !self.may_send(deploy_port_tree_id)? { return Err(CellagentError::MayNotSend { func_name: _f, cell_id: self.cell_id.clone(), tree_id: deploy_tree_id.clone() }.into()); }
        ...
        for allowed_tree in allowed_trees {
            tree_map
                ...
                .ok_or::<Error>(CellagentError::TreeMap { func_name: "listen_cm_loop 5", cell_id: self.cell_id.clone(), tree_name: allowed_tree.clone() }.into())?;
        }
    ...
    fn tcp_stack_tree(&mut self, sender_id: &SenderID, serialized: &str, direction: MsgDirection,
        ...
        let gvm_eqn = &serde_json::from_str::<GvmEquation>(&gvm_eqn_serialized).context(CellagentError::Chain { func_name: _f, comment: S(self.cell_id.clone()) + " gvm" })?;
        ...
        let entry = self.stack_tree(sender_id, &allowed_tree, &new_tree_id.to_port_tree_id_0(),
                                    ...
                                    None, gvm_eqn)?
            .ok_or::<Error>( CellagentError::StackTree { func_name: _f, cell_id: self.cell_id.clone(), tree_id: new_tree_id.to_port_tree_id_0() }.into())?;
        let allowed_tree = AllowedTree::new(&new_tree_name);
        ...
        let traph = self.get_traph_clone(&parent_port_tree_id).context(CellagentError::Chain { func_name: _f, comment: S("") })?;
    ...
    fn get_msg_params(&self, msg: &mut HashMap<String, String>, param: &str) -> Result<String, Error> {
        ...
        msg
            .get(param)
            .map(|param| S(param))
            .ok_or::<Error>(CellagentError::Message { func_name: "get_param", cell_id: self.cell_id.clone(), msg: msg_err_msg.clone() }.into())
    }
    fn gvm_eval_save(&self, tree_id: &PortTreeID, gvm_eqn: &GvmEquation) -> Result<bool, Error> {
        ...
        match self.get_traph_clone(tree_id) {
            Ok(traph) => {
    ...
    */
    ...
    fn port_disconnected(&mut self, port_no: PortNo) -> Result<(), Error> {
        ...
        let mut my_traph = self.own_traph(&my_port_tree_id)?;
        my_traph.set_broken(port_number);
        self.insert_traph(&my_port_tree_id.clone(), my_traph)?;
        ...
        let rw_port_tree_id = rw_traph.get_base_tree_id().to_port_tree_id_0(); // Need clone to avoid borrow problem
        let broken_path = {
        ...
        if let Some(trial_parent_port) = rw_traph.find_new_parent_port(&rw_port_tree_id, broken_path) {
            ...
            broken_base_tree_ids.remove(&rw_port_tree_id.to_tree_id());
            let port_number = trial_parent_port.make_port_number(self.no_ports)?;
            let mask = Mask::new(port_number);
            let failover_msg = FailoverMsg::new(&sender_id, &rw_port_tree_id, &lw_port_tree_id,
                                                broken_path, &broken_base_tree_ids);
    ...
    fn send_bytes(&self, tree_id: &TreeID, is_ait: bool, is_blocking: bool, user_mask: Mask,
        ...
        self.tree_map
            .get(&tree_uuid)
            .ok_or::<Error>(CellagentError::Tree { func_name: _f, cell_id: self.cell_id.clone(), tree_uuid }.into())?;
