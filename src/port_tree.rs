...
impl PortTree {
    pub fn new(tree_id: &TreeID, root_port_number: &PortNumber, in_port_no: &PortNo, hops: &PathLength) -> PortTree {
        let port_tree_id = tree_id.with_root_port_number(root_port_number);
        PortTree { tree_id: port_tree_id, root_port_no: root_port_number.get_port_no(),
                   in_port_no: *in_port_no, hops: *hops }
    }
    pub fn get_port_tree_id(&self) -> &TreeID { &self.tree_id }
...
impl fmt::Display for PortTree {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let s = format!("PortTree: TreeID {}, root_port {}, in_port {}, hops {}",
                        self.tree_id, *self.root_port_no, *self.in_port_no, self.hops);
        write!(f, "{}", s)
    }
}
