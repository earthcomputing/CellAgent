...
impl From<NameError> for TenantErrors {
	fn from(err: NameError) -> TenantErrors { TenantErrors::TenantNameError(err) }
}
impl From<TenantError> for TenantErrors {
	fn from(err: TenantError) -> TenantErrors { TenantErrors::TenantBuildError(err) }
}
...
impl TenantError { 
	...
	fn tenant_already_exists(id: &str) -> String { 
		format!("Tenant {} already exists.",id) 
	}
	fn not_enough_cells(n: usize, available: usize) -> String {
		format!("You asked for {} cells, but only {} available", n, available)
	}
}
